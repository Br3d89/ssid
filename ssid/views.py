    ...
    try:
        ...
        if action=='enable':
            ...
            if number_of_wlans < 16:
                ...
                for i in ssid_objects:
                    ...
                    child.expect(">")
                    child.sendline('config acl url-domain add facebook.net {}_social'.format(acl_shortcut))
                    child.expect(">")
                    child.sendline('config acl url-domain add vk.com {}_social'.format(acl_shortcut))
                    child.expect(">")
                    child.sendline('config acl url-domain add apple.com {}_apple'.format(acl_shortcut))
                    child.expect(">")
                    child.sendline('config acl url-domain add connectivitycheck.android.com {}_apple'.format(acl_shortcut))
                    child.expect(">")
                    child.sendline('config acl url-domain add clients3.google.com {}_apple'.format(acl_shortcut))
                    child.expect(">")
                    child.sendline('config acl url-domain add www.msftncsi.com {}_apple'.format(acl_shortcut))
                    child.expect(">")
                    child.sendline('config acl url-domain add appleiphonecell.com {}_apple'.format(acl_shortcut))
                    child.expect(">")
        ...
        if action=='disable':
            ...
            for i in ssid_objects:
                ...
                child.sendline('show radius summary')
                child.expect(">")
                r_s = str(child.before)
                print(r_s)
                radius_server_id=r_s.split(i.web.ip)[0].split(r'\r\n')[-1].split()[0]
