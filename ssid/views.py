    ...
    try:
        ...
        if action=='up/down':
            ...
            if number_of_wlans < 16:
                ...
                child.expect(">")
                child.sendline('config acl url-domain add fbcdn.net {}_social'.format(ssid_server.name, acl_shortcut))
                child.expect(">")
                child.sendline('config acl url-domain add facebook.com {}_social'.format(ssid_server.name, acl_shortcut))
                child.expect(">")
                child.sendline('config acl url-domain add facebook.net {}_social'.format(ssid_server.name, acl_shortcut))
                child.expect(">")
                child.sendline('config acl url-domain add vk.com {}_social'.format(ssid_server.name, acl_shortcut))
                child.expect(">")
                child.sendline('config acl url-domain add apple.com {}_apple'.format(ssid_server.name,acl_shortcut))
                child.expect(">")
                child.sendline('config acl url-domain add connectivitycheck.android.com {}_apple'.format(ssid_server.name, acl_shortcut))
                child.expect(">")
                child.sendline('config acl url-domain add clients3.google.com {}_apple'.format(ssid_server.name, acl_shortcut))
                child.expect(">")
                child.sendline('config acl url-domain add www.msftncsi.com {}_apple'.format(ssid_server.name, acl_shortcut))
                child.expect(">")
                child.sendline('config acl url-domain add appleiphonecell.com {}_apple'.format(ssid_server.name, acl_shortcut))
                child.expect(">")
                child.sendline('config acl url-domain add akamaitechnologies.com {}_apple'.format(ssid_server.name, acl_shortcut))
                child.expect(">")
